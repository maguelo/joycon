...
class Joycon(object):
    ...
    def __init__(self, vendor_id, product_id):
        ...
        self.STICK_OFFSET_H = 0
        self.STICK_OFFSET_V = 0
    ...
    def calibrate(self):
        ...
        self.ACCEL_OFFSET_X = int(sum(calibration['a_x']) / len(calibration['a_x']))
        self.ACCEL_OFFSET_Y = int(sum(calibration['a_y']) / len(calibration['a_y']))
        self.ACCEL_OFFSET_Z = int(sum(calibration['a_z']) / len(calibration['a_z']))

        self.GYRO_OFFSET_X = int(sum(calibration['g_x']) / len(calibration['g_x']))
        self.GYRO_OFFSET_Y = int(sum(calibration['g_y']) / len(calibration['g_y']))
        self.GYRO_OFFSET_Z = int(sum(calibration['g_z']) / len(calibration['g_z']))
        
        self.STICK_OFFSET_H = int(sum(calibration['s_h']) / len(calibration['s_h']))
        self.STICK_OFFSET_V = int(sum(calibration['s_v']) / len(calibration['s_v']))
